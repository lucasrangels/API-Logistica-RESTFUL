...
def check_point_in_polygon(geo_json: json, str_point):
    is_valid = False

    point = str(str_point).replace('(', '').replace(')', '').replace("'", '')
    print(point)
    lat, long = point.split(',', 1)
    point = Point(float(str(lat).replace(',', '')), float(str(long).replace(',', '')))

    location = json.loads(geo_json)
    c = [Point(coord[0], coord[1]) for coord in location['features'][0]["geometry"]['coordinates'][0]]
    poly = geometry.Polygon([[p.x, p.y] for p in c])
...
def check_geojson_integrity(obj):
    ...
    if isinstance(obj, geojson.MultiPoint):
        if check_list_of_objects(obj['coordinates'], lambda x: len(x) == 2):
            return output(
    ...
    if isinstance(obj, geojson.MultiLineString):
        ...
        if check_list_of_objects(coord, lambda x: len(x) >= 2):
            return output('the "coordinates" member must be an array '
    ...
    if isinstance(obj, geojson.MultiPolygon):
        if check_list_of_objects(obj['coordinates'],
...
def check_list_of_objects(coord, pred):
